...
The research question of this paper is to determine if open-source language models can be used to clean and analyze open-ended survey questions. The goal is to provide a practical framework for transforming the analysis of open-ended questions in survey research. By leveraging open-source tools, we aim to enhance the ability to extract meaningful insights from survey responses, providing a scalable solution for researchers. The goal is not to replace other methods of analyzing open-ended questions but to provide and validate an alternative that could be used in conjunction with other methods to improve the quality of the analysis.
...
To validate if open-source LLMs with limited parameters can be used to clean open-ended questions, we will run issue categorization on a subset of 200 respondents drawn randomly from the 2021 Canadian Election Study. The respondents were asked "What is the most important issue to you personally in this federal election?" and could answer freely.

For this task, the open-source Mistral model by @jiang_etal23 was used to analyze the answer and classify it into one of the following 12 issue categories: "Law and Crime", "Culture and Nationalism", "Public Lands and Agriculture", "Governments and Governance", "Immigration", "Rights, Liberties, Minorities, and Discrimination", "Health and Social Services", "Economy and Employment", "Education", "Environment and Energy". "International Affairs and Defense", "Technology"

The model was run on a desktop with 16GB of RAM, a Intel i5-4690K CPU and a GTX 1660 TI GPU. Relatively old hardware that is still able to run the model without any issues.
