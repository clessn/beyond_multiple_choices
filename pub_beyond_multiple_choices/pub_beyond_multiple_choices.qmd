...
To test the accuracy of each models, the percentage of correct categorization was calculated. The results are shown in @fig-accuracy. The larger model, GPT-4-Turbo, scored the highest with 84% accuracy. Llama3 scored at 79.5%, followed by Phi3 at 75.5%, and Mistral at 73.5%. The dictionnary analysis scored the lowest at 19%. Those results are in line with the expectations: The larger model, GPT-4-Turbo, is known to be the most capable model available on the market at the moment of writing this paper. The smaller models, Llama3, Phi3, and Mistral, are open-source models with limited parameters. They are not as capable as GPT-4-Turbo but are still able to perform the task. The dictionnary analysis is limited in scope and does not perform well in short open-ended questions. 
