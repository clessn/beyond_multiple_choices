...
data <- data %>%
  mutate(issue_comparison_mistral = case_when(
    is.na(issue_category_human) & is.na(issue_category_mistral) ~ 1,
    is.na(issue_category_human) | is.na(issue_category_mistral) ~ 0,
    str_to_lower(issue_category_human) == str_to_lower(issue_category_mistral) ~ 1,
    TRUE ~ 0
  ))
...
data <- data %>%
  mutate(issue_comparison_phi3 = case_when(
    is.na(issue_category_human) & is.na(issue_category_phi3) ~ 1,
    is.na(issue_category_human) | is.na(issue_category_phi3) ~ 0,
    str_to_lower(issue_category_human) == str_to_lower(issue_category_phi3) ~ 1,
    TRUE ~ 0
  ))
...
table(data$issue_comparison_mistral, useNA = "ifany")
table(data$issue_comparison_llama3, useNA = "ifany")
table(data$issue_comparison_phi3, useNA = "ifany")
